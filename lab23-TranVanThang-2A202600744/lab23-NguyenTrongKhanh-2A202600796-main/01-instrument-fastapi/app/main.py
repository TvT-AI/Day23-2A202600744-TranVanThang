...
def predict(req: PredictRequest) -> PredictResponse:
    ...
    try:
        # The root must be current so all three stage spans share one trace.
        # Unhandled HTTPException also marks this span ERROR, which is required
        # for the collector's keep-all-errors tail-sampling policy.
        with tracer.start_as_current_span("POST /predict") as span:
            span.set_attribute("gen_ai.operation.name", "chat")
            span.set_attribute("gen_ai.request.model", req.model)
            span.set_attribute("gen_ai.system", "llama.cpp-mock")
            ...
            with tracer.start_as_current_span("vector-search") as s:
                s.set_attribute("db.system", "qdrant")
